import java.util.ArrayList;

public class Library {
    private ArrayList<Book> books = new ArrayList<>();
    private ArrayList<User> borrowed = new ArrayList<>();

    public void addBook(Book book){
        books.add(book);
    }

    public void displayBooks(){
        System.out.println("All books: ");
        for(Book book : books){
            System.out.printf("Title: %-40s    Author: %-20s    Publication year %-5s   Available: %-10b\n", book.getTitle(), book.getAuthor(), book.getPublication(), book.isAvailable());
        }
    }

    public void findBooksByAuthor(String author){
        System.out.printf("Books by %s:\n", author);
        for(Book book : books){
            if(book.getAuthor().equals(author)){
                System.out.printf("Title: %s    Publication year %s\n", book.getTitle(), book.getPublication());
            }

        }
    }

    public void borrowBook(String title, User user){
        for(Book book : books){
            if(book.getTitle().equals(title)){
                book.setAvailable(false);
                borrowed.add(user);
                user.addBook(book);
                book.setOwner(user);
                return;
            }
        }
    }

    public void returnBook(Book book){
        if(books.contains(book)){
            book.setAvailable(true);
            User user = book.getOwner();
            user.returnBook(book);
            borrowed.remove(user);
        }
    }

    public boolean isBookAvailable(String title){
        for(Book book : books){
            if(book.getTitle().equals(title)){
                return book.isAvailable();
            }
        }
        return false;
    }

    public double getAverageBookRating(){
        int i = 0;
        double total = 0;
        for(Book book : books){
            if(book.getRating()>0){
                total += book.getRating();
                i++;
            }
        }
        return total/i;
    }

    public String getMostReviewedBook(){
        String result = "";
        int maxVal = 0;
        for(Book book : books){
            if(book.getReviews()>maxVal){
                result = book.getTitle();
                maxVal = book.getReviews();
            }
        }
        return result;
    }

    public void borrowedByUser(){
        for(User user : borrowed){
            System.out.println(user.getName());
            for (Book book : user.getBorrowed()){
                System.out.printf("         Title: %-40s Author: %-20s\n", book.getTitle(), book.getAuthor());
            }
            }
        }


}